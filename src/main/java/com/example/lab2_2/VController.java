...
public class VController {
    ...
    @FXML
    private TableColumn<DiaryB, String> columnWorking;
    @FXML
    ...
    public void initialize() {
       ...
       // columnDate.setCellValueFactory(new PropertyValueFactory<>("date"));
        ...
        columnWorking.setCellValueFactory(new PropertyValueFactory<>("working"));
       ...
       // columnDate.setCellFactory(TextFieldTableCell.forTableColumn());
        columnWorking.setCellFactory(TextFieldTableCell.forTableColumn());
        ...
        columnStatus.setCellFactory(col -> {
            ...
            cell.itemProperty().addListener((obs, oldItem, newItem) -> {
                if (newItem == null || newItem.isEmpty()) {
                    ...
                    if (normalized.contains("не відпрацьовано") || normalized.contains("not done")) {
                        cell.setStyle("-fx-background-color: #FF6347; -fx-text-fill: white;");
                    } else if (normalized.contains("відпрацьовано") || normalized.contains("done")) {
        ...
        columnWorking.setOnEditCommit(event -> {
            event.getRowValue().setWorking(event.getNewValue());
            db.update(event.getRowValue());
        });
        ...
        // Відображаємо тільки записи категорії "visiting"
        refreshTable();
    ...
    public void showDialog(DiaryB diaryB, Stage owner) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("visitingAdd.fxml"));
            Parent root = loader.load();
            VAddController controller = loader.getController();
    ...
    void openWindow(ActionEvent event) {
        ...
        switch (clickedButton.getId()) {
            case "bthAdd":
                DiaryB newDiary = new DiaryB("", "", "", "", ""); // категорія "visiting"
                newDiary.setCategory("visiting");
            ...
            case "search":
                ...
                ObservableList<DiaryB> filtered = db.getAll().filtered(diary ->
                        "visiting".equalsIgnoreCase(diary.getCategory()) && (
                                diary.getSubject().toLowerCase().contains(query) ||
                                        diary.getDate().toLowerCase().contains(query) ||
                                        diary.getWorking().toLowerCase().contains(query) ||
    ...
    private void refreshTable() {
        ObservableList<DiaryB> visitingList = db.getAll().filtered(diary ->
                "visiting".equalsIgnoreCase(diary.getCategory())
        );
        diaryTable.setItems(visitingList);
    }
